    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-12">
      <Card className="w-full max-w-md border-none shadow-lg">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>Connectez-vous pour continuer sur StyleÉchange</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {process.env.NEXT_PUBLIC_GOOGLE_ENABLED === "true" && (
            <>
              <Button variant="outline" className="w-full h-12 gap-2" onClick={handleGoogleSignIn} disabled={isLoading}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-12 bg-accent text-accent-foreground hover:bg-accent/90 font-semibold" disabled={isLoading}>
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/auth/register" className="text-accent hover:underline font-semibold">S'inscrire</Link>
          </p>
        </CardContent>
      </Card>
    </div>
